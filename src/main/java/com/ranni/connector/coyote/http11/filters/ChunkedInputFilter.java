...
import com.ranni.connector.coyote.InputBuffer;
import com.ranni.connector.coyote.Request;
import com.ranni.connector.coyote.http11.Constants;
import com.ranni.connector.coyote.http11.InputFilter;
...
public class ChunkedInputFilter implements InputFilter, ApplicationBufferHandler {
    ...
    public int doRead(ApplicationBufferHandler handler) throws IOException {
        ...
        if (remaining <= 0) {
            if (!parseChunkHeader()) {
                throwIOException("chunkedInputFilter.invalidHeader");
            }
            if (endChunk) {
                parseEndChunk();
                return -1;
        ...
        if (readChunk == null || readChunk.position() >= readChunk.limit()) {
            if (readBytes() < 0) {
                throwIOException("chunkedInputFilter.eos");
            }
        }
    ...
    public long end() throws IOException {
        ...
        while ((read = doRead(this)) >= 0) {
            ...
            if (maxSwallowSize > -1 && swallowed > maxSwallowSize) {
                throwIOException("inputFilter.maxSwallow");
            }
        }
        ...
        while (!eol) {
            ...
            if (chr == Constants.CR || chr == Constants.LF) {
                ...
                if (maxExtensionSize > -1 && extensionSize > maxExtensionSize) {
                    throwIOException("chunkedInputFilter.maxExtension");
                }
            }
    ...
    protected void parseCRLF(boolean tolerant) throws IOException {
        ...
        while (!eol) {
            if (readChunk == null || readChunk.position() >= readChunk.limit()) {
                if (readBytes() <= 0) {
                    throwIOException("chunkedInputFilter.invalidCrlfNoData");
                }
            }
            ...
            if (chr == Constants.CR) {
                if (crfound) {
                    throwIOException("chunkedInputFilter.invalidCrlfCRCR");
                }
                crfound = true;
            } else if (chr == Constants.LF) {
                if (!tolerant && !crfound) {
                    throwIOException("chunkedInputFilter.invalidCrlfNoCR");
                }
                eol = true;
            } else {
                throwIOException("chunkedInputFilter.invalidCrlf");
            }
    ...
    private boolean parseHeader() throws IOException {
        ...
        if (readChunk == null || readChunk.position() >= readChunk.limit()) {
            if (readBytes() <0) {
               throwEOFException("chunkedInputFilter.eosTrailer");
            }
        }
            ...
            while (space) {
                ...
                if ((chr == Constants.SP) || (chr == Constants.HT)) {
                    ...
                    if (trailingHeaders.getEnd() > newlimit) {
                        throwIOException("chunkedInputFilter.maxTrailer");
                    }
    ...
    private void checkError() throws IOException {
        if (error) {
            throw new IOException("chunkedInputFilter.error");
        }
    }
