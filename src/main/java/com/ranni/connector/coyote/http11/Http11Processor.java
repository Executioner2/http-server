...
import com.ranni.connector.coyote.*;
import com.ranni.connector.coyote.http11.filters.*;
...
//        System.out.println("Http11Processor#service()");
        ...
        while (!getErrorState().isError() && keepAlive && !isAsync() 
            ...
            if (getErrorState().isIoAllowed()) {
                ri.setStage(com.ranni.connector.coyote.Constants.STAGE_PREPARE);
                try {
                    prepareRequest();
                } catch (Throwable t) {
            ...
            if (getErrorState().isIoAllowed()) {
                try {
                    ri.setStage(com.ranni.connector.coyote.Constants.STAGE_SERVICE);
            ...
            // 换成请求处理
            ri.setStage(com.ranni.connector.coyote.Constants.STAGE_ENDINPUT);
            if (!isAsync()) {
                // 同时，也会完成输出缓冲区的flush
                endRequest();
            }
            // 完成响应
            ri.setStage(com.ranni.connector.coyote.Constants.STAGE_ENDOUTPUT);
            ...
            ri.setStage(com.ranni.connector.coyote.Constants.STAGE_KEEPALIVE);
//            sendfileState = processSendfile(socketWrapper);
            
        } // while end
        
        ri.setStage(com.ranni.connector.coyote.Constants.STAGE_ENDED);
...
//        }
    ...
    private void prepareSendfile(OutputFilter[] outputFilters) {
        String fileName = (String) request.getAttribute(com.ranni.connector.coyote.Constants.SENDFILE_FILE_END_ATTR);
        if (fileName == null) {
            sendfileData = null;
        } else {
            outputBuffer.addActiveFilter(outputFilters[Constants.VOID_FILTER]);
            contentDelimitation = true;
            long pos = (long) request.getAttribute(com.ranni.connector.coyote.Constants.SENDFILE_FILE_START_ATTR);
            long end = (long) request.getAttribute(com.ranni.connector.coyote.Constants.SENDFILE_FILE_END_ATTR);
